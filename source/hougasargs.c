...
#define mjhargsc(chart,n) (*((int**)chart)[n*2+1])
#define mjhargsv(chart,n) (((char**)chart)[n*2])
...
int hougasargs(int argc, char** argv)
{
 ...
 mjhargsproc(&chart,validflags,argc,argv);

 for(i=0;i<nvalid;i+=2) printf("%s %s %s %d\n",validflags[i],validflags[i+1],mjhargsv(chart,i/2),mjhargsc(chart,i/2));
}
